...
const ESTADO_BADGE: Record<string, string> = {
  borrador: "badge-borrador", solicitado: "badge-solicitado",
  aprobado: "badge-aprobado", pago_pendiente: "badge-pago",
  publicado: "badge-publicado", rechazado: "badge-rechazado",
};

const ESTADO_LABEL: Record<string, string> = {
  borrador: "Borrador", solicitado: "Solicitado", aprobado: "Aprobado",
  pago_pendiente: "Pago pendiente", publicado: "Publicado", rechazado: "Rechazado",
};

const FORM_VACIO = {
  titulo: "", descripcion: "", fecha: "", lugar: "", link_externo: "",
  tipo: "gfi", gratuito: true, precio_entrada: "", capacidad: "",
  imagen_url: "", video_url: "", costo_publicacion: "",
};

// Enviar push notification a todos los suscriptores
async function enviarPushEvento(evento: Evento) {
  try {
    ...
    await fetch("/api/push/send", {
      ...
      body: JSON.stringify({
        titulo: `📅 Nuevo evento GFI®: ${evento.titulo}`,
        body: `${evento.lugar ? `📍 ${evento.lugar} · ` : ""}${fecha}${evento.gratuito ? " · Gratuito" : ""}`,
        url: "/eventos",
        tipo: "eventos",
      }),
    });
...
export default function AdminEventosPage() {
  ...
  const [pushEnviado, setPushEnviado] = useState<string | null>(null);
  ...
  const guardarEvento = async () => {
    ...
    const payload = {
      ...
      fecha: new Date(form.fecha).toISOString(), lugar: form.lugar || null,
      link_externo: form.link_externo || null, tipo: form.tipo,
      ...
      imagen_url: form.imagen_url || null, video_url: form.video_url || null,
      estado: "borrador",
    };
    if (editandoId) {
  ...
  const cambiarEstado = async (id: string, estado: string) => {
    ...
    // Si se publica → enviar push automáticamente
    if (estado === "publicado") {
      const ev = eventos.find(e => e.id === id);
      if (ev) {
        await enviarPushEvento({ ...ev, estado: "publicado" });
        setPushEnviado(id);
        setTimeout(() => setPushEnviado(null), 4000);
      }
    }
  ...
  const confirmarPago = async (id: string) => {
    ...
    // Enviar push al confirmar pago (también publica)
    const ev = eventos.find(e => e.id === id);
    if (ev) {
      await enviarPushEvento({ ...ev, estado: "publicado" });
      setPushEnviado(id);
  ...
  const editarEvento = (ev: Evento) => {
    setForm({
      ...
      lugar: ev.lugar ?? "", link_externo: ev.link_externo ?? "",
      tipo: ev.tipo, gratuito: ev.gratuito,
      precio_entrada: ev.precio_entrada?.toString() ?? "",
      capacidad: ev.capacidad?.toString() ?? "",
      imagen_url: ev.imagen_url ?? "", video_url: ev.video_url ?? "",
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .ae-topbar { display: flex; align-items: center; justify-content: space-between; padding: 0 32px; height: 60px; background: rgba(14,14,14,0.98); border-bottom: 1px solid rgba(180,0,0,0.2); position: sticky; top: 0; z-index: 100; }
        ...
        .ae-filtro { padding: 7px 14px; background: rgba(14,14,14,0.9); border: 1px solid rgba(255,255,255,0.1); border-radius: 3px; font-family: 'Montserrat', sans-serif; font-size: 10px; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; color: rgba(255,255,255,0.4); cursor: pointer; transition: all 0.2s; display: flex; gap: 6px; align-items: center; }
        ...
        .ae-push-badge { font-size: 9px; padding: 2px 8px; background: rgba(34,197,94,0.12); border: 1px solid rgba(34,197,94,0.3); border-radius: 20px; color: #22c55e; font-family: 'Montserrat', sans-serif; font-weight: 700; letter-spacing: 0.08em; white-space: nowrap; animation: fadeIn 0.3s ease; }
        ...
        .ae-modal-bg { position: fixed; inset: 0; background: rgba(0,0,0,0.8); display: flex; align-items: center; justify-content: center; z-index: 200; padding: 24px; }
        .ae-modal { background: #0f0f0f; border: 1px solid rgba(180,0,0,0.25); border-radius: 6px; padding: 36px; width: 100%; max-width: 560px; max-height: 90vh; overflow-y: auto; position: relative; }
        .ae-modal::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px; background: linear-gradient(90deg, transparent, #cc0000, transparent); border-radius: 6px 6px 0 0; }
      ...
      <div className="ae-root">
        <header className="ae-topbar">
          ...
          <div className="ae-topbar-right">
            ...
            <button className="ae-btn-nuevo" onClick={() => { setForm(FORM_VACIO); setEditandoId(null); setMostrarForm(true); }}>+ Nuevo evento</button>
          </div>
        </header>
        ...
        <main className="ae-content">
          <div className="ae-header">
            ...
            <p>Creá, aprobá y publicá eventos. Al publicar se envía push automático a todos los suscriptores.</p>
          </div>
          ...
          <div className="ae-tabla-wrap">
            {loading ? (
              ...
              <table className="ae-tabla">
                <thead>
                  <tr><th>Evento</th><th>Tipo</th><th>Fecha</th><th>Estado</th><th>Acciones</th></tr>
                </thead>
                <tbody>
                    ...
                    <tr key={ev.id}>
                      ...
                      <td>
                        <div style={{display:"flex",flexDirection:"column",gap:4}}>
                          <span className={`badge ${ESTADO_BADGE[ev.estado]}`}>{ESTADO_LABEL[ev.estado]}</span>
                          {pushEnviado === ev.id && <span className="ae-push-badge">🔔 Push enviado</span>}
                        </div>
                      </td>
                      <td>
                        {procesando === ev.id ? (
                          ...
                          <div className="ae-acciones">
                            ...
                            {ev.estado === "borrador" && (
                              <button className="ae-btn ae-btn-pub" onClick={() => cambiarEstado(ev.id, "publicado")}>
                                🔔 Publicar + Push
                              </button>
                            )}
                            {ev.estado === "solicitado" && (
                              <>
                                {costoPendiente?.id === ev.id ? (
                                  <>
                                    <input className="ae-costo-input" placeholder="$ costo" value={costoPendiente.valor} onChange={e => setCostoPendiente({ id: ev.id, valor: e.target.value })} />
                                  ...
                                  <button className="ae-btn ae-btn-pub" onClick={() => setCostoPendiente({ id: ev.id, valor: "" })}>Aprobar + costo</button>
                                )}
                                <button className="ae-btn ae-btn-rech" onClick={() => cambiarEstado(ev.id, "rechazado")}>Rechazar</button>
                              </>
                            )}
                            {ev.estado === "pago_pendiente" && (
                              <button className="ae-btn ae-btn-pago" onClick={() => confirmarPago(ev.id)}>✓ Pago + Push</button>
                            )}
                            {ev.estado === "aprobado" && (
                              <button className="ae-btn ae-btn-pub" onClick={() => cambiarEstado(ev.id, "publicado")}>🔔 Publicar + Push</button>
                            )}
                            {ev.estado === "publicado" && (
          ...
          <div className="ae-modal">
            ...
            <div className="ae-form-field">
              ...
              <div className="ae-toggle">
                ...
                {!form.gratuito && <input className="ae-form-input" type="number" placeholder="Precio $" style={{maxWidth:120}} value={form.precio_entrada} onChange={e => handleForm("precio_entrada", e.target.value)} />}
              </div>
            </div>
            <div className="ae-form-field">
