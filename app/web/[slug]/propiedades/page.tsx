            ...
            {propiedades.map(p => (
              <a key={p.id} href={`/web/${cfg.slug}/propiedad/${p.id}`} className="prop-card" style={p.destacada_web ? { borderColor: `${t.accent}60` } : {}}>
                <div className="prop-img">
                  ...
                  {p.destacada_web && (
                    <div style={{ position: "absolute", top: 8, left: 8, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(4px)", borderRadius: 4, padding: "2px 7px", fontSize: 9, fontWeight: 700, color: "#d4960c", fontFamily: "Montserrat,sans-serif", letterSpacing: "0.06em", display: "flex", alignItems: "center", gap: 3 }}>
                      ⭐ DESTACADA
                    </div>
                  )}
                </div>
