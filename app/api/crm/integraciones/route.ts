...
export async function GET(req: NextRequest) {
  ...
  // Default: logs + configs
  const [logs, configs] = await Promise.all([
    supabase.from("crm_integraciones_log").select("*").eq("perfil_id", user.id).order("created_at", { ascending: false }).limit(50),
    supabase.from("crm_integraciones_config").select("tipo,activo,ultima_sincronizacion,created_at").eq("perfil_id", user.id),
  ]);
...
export async function POST(req: NextRequest) {
  ...
  if (accion === "guardar_config") {
    ...
    const { error } = await supabase.from("crm_integraciones_config").upsert(
  ...
  if (accion === "update_sync") {
    ...
    await supabase.from("crm_integraciones_config").update({ ultima_sincronizacion: new Date().toISOString() })
  ...
  if (accion === "importar_contactos") {
    ...
    await supabase.from("crm_integraciones_log").insert({
      perfil_id: user.id,
  ...
  if (accion === "importar_propiedades") {
    ...
    for (const p of propiedades) {
      ...
      const { error } = await supabase.from("cartera_propiedades").insert({
        perfil_id:          user.id,
