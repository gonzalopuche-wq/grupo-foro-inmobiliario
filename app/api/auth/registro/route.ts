...
export async function POST(req: NextRequest) {
  ...
  const { email, password, tipo, nombre, apellido, matricula, dni, telefono, inmobiliaria, especialidades } = body;
  ...
  const { error: perfilError } = await sb.from("perfiles").insert({ id: userId, tipo, estado: "pendiente", nombre, apellido, matricula: tipo === "corredor" ? matricula : null, dni: tipo === "colaborador" ? dni : null, telefono: telefono || null, inmobiliaria: tipo === "corredor" ? (inmobiliaria || null) : null, especialidades: tipo === "colaborador" ? especialidades : null });
