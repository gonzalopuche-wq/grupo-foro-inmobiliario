    ...
    <div style={{ fontFamily: "Inter,sans-serif", color: "#fff", maxWidth: 1000, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: 20, fontWeight: 800, fontFamily: "var(--font-display)" }}>Cursos y Capacitación</h1>
            <p style={{ margin: 0, fontSize: 12, color: "var(--gfi-text-muted)" }}>Formación continua para corredores GFI®</p>
          </div>
        </div>
        {esAdmin && (
      ...
      {loading ? (
        <div style={{ textAlign: "center", padding: 60, color: "var(--gfi-text-muted)" }}>Cargando cursos...</div>
              ...
              <div key={c.id} className="curso-card" onClick={() => setCursoVer(c)}>
                ...
                <div style={{ padding: "14px 16px" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
                    ...
                    <span style={{ fontSize: 10, color: "var(--gfi-text-muted)" }}>{NIVELES[c.nivel] ?? c.nivel}</span>
                  </div>
                  <h3 style={{ margin: "0 0 6px", fontSize: 14, fontWeight: 700, fontFamily: "var(--font-display)", lineHeight: 1.3 }}>{c.titulo}</h3>
                  {c.instructor && <p style={{ margin: "0 0 8px", fontSize: 11, color: "rgba(255,255,255,0.45)" }}>👤 {c.instructor}</p>}
                  {c.descripcion && <p style={{ margin: "0 0 10px", fontSize: 12, color: "var(--gfi-text-secondary)", lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{c.descripcion}</p>}
                  ...
                  {inscripto && (
                    <div style={{ marginTop: 8 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "var(--gfi-text-muted)", marginBottom: 3 }}>
                        <span>{completado ? "✓ Completado" : `Progreso: ${c.mi_inscripcion!.progreso}%`}</span>
                      </div>
                      <div style={{ height: 4, background: "var(--gfi-border)", borderRadius: 2, overflow: "hidden" }}>
      ...
      {cursoVer && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}
          ...
          <div style={{ background: "var(--gfi-bg-secondary)", border: "1px solid var(--gfi-border)", borderRadius: 18, width: "100%", maxWidth: 560, maxHeight: "90vh", overflow: "auto", position: "relative" }}>
            ...
            <div style={{ padding: 24 }}>
              <button style={{ position: "absolute", top: 12, right: 12, background: "rgba(0,0,0,0.7)", border: "1px solid var(--gfi-border)", borderRadius: "50%", width: 32, height: 32, color: "var(--gfi-text-primary)", fontSize: 16, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setCursoVer(null)}>&times;</button>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
                ...
                {cursoVer.duracion_horas && <span style={{ fontSize: 11, color: "var(--gfi-text-muted)" }}>⏱ {cursoVer.duracion_horas}hs</span>}
                <span style={{ fontSize: 11, color: "var(--gfi-text-muted)", textTransform: "capitalize" }}>{cursoVer.modalidad}</span>
              </div>

              <h2 style={{ margin: "0 0 8px", fontSize: 18, fontWeight: 800, fontFamily: "var(--font-display)" }}>{cursoVer.titulo}</h2>
              {cursoVer.instructor && <p style={{ margin: "0 0 12px", fontSize: 13, color: "var(--gfi-text-secondary)" }}>👤 {cursoVer.instructor}</p>}
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
                <div style={{ fontSize: 20, fontWeight: 800, color: cursoVer.gratuito ? "#3abab6" : "#d4960c", fontFamily: "var(--font-display)" }}>
                  {cursoVer.gratuito ? "Gratuito" : `${cursoVer.precio.toLocaleString("es-AR")} ${cursoVer.moneda}`}
                </div>
                {cursoVer.max_inscriptos && <div style={{ fontSize: 12, color: "var(--gfi-text-muted)" }}>Cupos: {cursoVer.max_inscriptos}</div>}
              </div>
              ...
              {cursoVer.mi_inscripcion && (
                <div style={{ marginBottom: 16, background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.2)", borderRadius: 8, padding: "10px 14px" }}>
                  ...
                  <div style={{ height: 6, background: "var(--gfi-border)", borderRadius: 3 }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                {cursoVer.link_acceso && (cursoVer.mi_inscripcion || cursoVer.gratuito) && (
                  <a href={cursoVer.link_acceso} target="_blank" rel="noopener noreferrer" style={{ flex: 1, padding: "11px", background: "#990000", border: "none", borderRadius: 8, color: "#fff", fontSize: 13, fontWeight: 700, textDecoration: "none", textAlign: "center", fontFamily: "var(--font-display)" }}>
                    Acceder al curso →
                  </a>
                )}
                <button
                  ...
                  style={{
                    ...
                    background: cursoVer.mi_inscripcion ? "transparent" : "var(--gfi-border)",
      ...
      {mostrarForm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, overflowY: "auto" }}
          ...
          <div style={{ background: "var(--gfi-bg-secondary)", border: "1px solid var(--gfi-border)", borderRadius: 16, padding: 28, width: "100%", maxWidth: 560, position: "relative" }}>
            <button style={{ position: "absolute", top: 12, right: 12, background: "none", border: "none", color: "var(--gfi-text-muted)", fontSize: 20, cursor: "pointer" }} onClick={() => setMostrarForm(false)}>&times;</button>
            <h3 style={{ margin: "0 0 20px", fontSize: 16, fontWeight: 800, fontFamily: "var(--font-display)" }}>Nuevo curso</h3>
            <div style={{ display: "grid", gap: 12 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <label style={{ fontSize: 10, color: "var(--gfi-text-muted)", display: "block", marginBottom: 4 }}>Fecha inicio</label>
                  <input className="cur-input" type="date" value={form.fecha_inicio} onChange={e => setF("fecha_inicio", e.target.value)} />
                </div>
                <div>
                  <label style={{ fontSize: 10, color: "var(--gfi-text-muted)", display: "block", marginBottom: 4 }}>Fecha fin</label>
                  <input className="cur-input" type="date" value={form.fecha_fin} onChange={e => setF("fecha_fin", e.target.value)} />
                </div>
              </div>
            </div>
            <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
              <button onClick={() => setMostrarForm(false)} style={{ flex: 1, padding: "10px", background: "var(--gfi-border-subtle)", border: "1px solid var(--gfi-border)", borderRadius: 8, color: "var(--gfi-text-secondary)", cursor: "pointer", fontSize: 13 }}>Cancelar</button>
