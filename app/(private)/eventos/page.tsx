...
export default function EventosPage() {
  ...
  return (
    <>
      <style>{`
        .ev-header { display: flex; align-items: flex-end; justify-content: space-between; flex-wrap: wrap; gap: 16px; margin-bottom: 24px; }
        .ev-header-text h2 { font-family: 'Montserrat', sans-serif; font-size: 18px; font-weight: 800; }
        .ev-header-text h2 span { color: #cc0000; }
        .ev-header-text p { font-size: 12px; color: rgba(255,255,255,0.35); margin-top: 4px; }
        .ev-proponer-btn { padding: 9px 18px; background: rgba(200,0,0,0.1); border: 1px solid rgba(200,0,0,0.35); border-radius: 3px; color: #cc0000; font-family: 'Montserrat', sans-serif; font-size: 10px; font-weight: 700; letter-spacing: 0.14em; text-transform: uppercase; cursor: pointer; text-decoration: none; transition: all 0.2s; }
        .ev-proponer-btn:hover { background: rgba(200,0,0,0.2); color: #fff; }
        .ev-filtros { display: flex; gap: 8px; margin-bottom: 20px; flex-wrap: wrap; }
        .ev-filtro { padding: 7px 16px; background: rgba(14,14,14,0.9); border: 1px solid rgba(255,255,255,0.1); border-radius: 3px; cursor: pointer; font-family: 'Montserrat', sans-serif; font-size: 10px; font-weight: 700; letter-spacing: 0.14em; text-transform: uppercase; color: rgba(255,255,255,0.4); transition: all 0.2s; }
        ...
        .ev-card { background: rgba(14,14,14,0.9); border: 1px solid rgba(255,255,255,0.07); border-radius: 6px; overflow: hidden; display: grid; grid-template-columns: 72px 1fr auto; transition: border-color 0.2s; animation: fadeUp 0.35s cubic-bezier(0.22,1,0.36,1) both; }
        .ev-card:hover { border-color: rgba(200,0,0,0.2); }
        .ev-fecha-col { background: rgba(200,0,0,0.08); border-right: 1px solid rgba(200,0,0,0.15); display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 14px 8px; gap: 2px; }
        .ev-fecha-num { font-family: 'Montserrat', sans-serif; font-size: 26px; font-weight: 800; color: #cc0000; line-height: 1; }
        .ev-fecha-mes { font-family: 'Montserrat', sans-serif; font-size: 9px; font-weight: 700; letter-spacing: 0.12em; color: rgba(255,255,255,0.4); }
        .ev-info { padding: 14px 18px; display: flex; flex-direction: column; gap: 6px; }
        .ev-info-top { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
        .ev-titulo { font-family: 'Montserrat', sans-serif; font-size: 14px; font-weight: 700; color: #fff; }
        ...
        .ev-acciones { padding: 14px; display: flex; flex-direction: column; align-items: flex-end; justify-content: center; gap: 8px; min-width: 130px; }
        .ev-inscribirse { padding: 8px 14px; background: #cc0000; border: none; border-radius: 3px; color: #fff; font-family: 'Montserrat', sans-serif; font-size: 10px; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; cursor: pointer; transition: all 0.2s; white-space: nowrap; }
        .ev-inscribirse:hover:not(:disabled) { background: #e60000; }
        .ev-inscribirse.inscripto { background: rgba(34,197,94,0.1); border: 1px solid rgba(34,197,94,0.3); color: #22c55e; }
        .ev-inscribirse.inscripto:hover:not(:disabled) { background: rgba(34,197,94,0.2); }
        .ev-inscribirse:disabled { opacity: 0.5; cursor: not-allowed; }
        .ev-inscriptos { font-size: 10px; color: rgba(255,255,255,0.25); text-align: right; }
        ...
        .ev-spinner { display: inline-block; width: 12px; height: 12px; border: 2px solid rgba(255,255,255,0.2); border-top-color: #fff; border-radius: 50%; animation: spin 0.7s linear infinite; }
        @keyframes spin { to { transform: rotate(360deg); } }
        @keyframes fadeUp { from { opacity: 0; transform: translateY(6px); } to { opacity: 1; transform: translateY(0); } }
        .ev-empty { padding: 56px 32px; text-align: center; color: rgba(255,255,255,0.2); font-size: 13px; font-style: italic; }
        @media (max-width: 600px) {
          .ev-card { grid-template-columns: 56px 1fr; }
          .ev-acciones { grid-column: 1 / -1; flex-direction: row; justify-content: space-between; padding: 10px 14px; border-top: 1px solid rgba(255,255,255,0.06); }
        }
      `}</style>
      ...
      {loading ? (
        <div className="ev-empty">Cargando eventos...</div>
        ...
        <div className="ev-lista">
          {eventosFiltrados.map((ev, i) => {
            ...
            const tipoBadge = TIPO_BADGE[ev.tipo] ?? { label: ev.tipo, color: "badge-externo" };
            return (
              <div
                key={ev.id}
                className="ev-card"
                style={{ animationDelay: `${i * 0.05}s`, opacity: pasado ? 0.5 : 1 }}
              >
                ...
                <div className="ev-info">
                  <div className="ev-info-top">
                    ...
                    <span className={`badge ${tipoBadge.color}`}>{tipoBadge.label}</span>
