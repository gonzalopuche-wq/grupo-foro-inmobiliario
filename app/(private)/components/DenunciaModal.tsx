...
export default function DenunciaModal({ tipoContenido, contenidoId, onClose, onSuccess }: Props) {
  ...
  return (
    ...
    >
      <div style={{ background: "var(--gfi-bg-secondary)", border: "1px solid var(--gfi-border)", borderRadius: 16, padding: 24, width: "100%", maxWidth: 420, position: "relative" }}>
        <button style={{ position: "absolute", top: 12, right: 12, background: "none", border: "none", color: "var(--gfi-text-muted)", fontSize: 20, cursor: "pointer" }} onClick={onClose}>&times;</button>

        {enviado ? (
          <div style={{ textAlign: "center", padding: "20px 0" }}>
            <div style={{ fontSize: 44, marginBottom: 12 }}>✅</div>
            <h3 style={{ margin: "0 0 8px", fontFamily: "var(--font-display)", fontSize: 16, fontWeight: 800 }}>Denuncia enviada</h3>
            <p style={{ margin: "0 0 20px", fontSize: 13, color: "var(--gfi-text-secondary)", lineHeight: 1.5 }}>
              Tu reporte fue recibido. El equipo de moderación GFI® lo revisará a la brevedad.
            </p>
            <button onClick={onClose} style={{ padding: "10px 24px", background: "var(--gfi-border)", border: "1px solid var(--gfi-border)", borderRadius: 8, color: "#fff", fontSize: 13, cursor: "pointer" }}>Cerrar</button>
          </div>
        ) : (
          <>
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 20, marginBottom: 6 }}>🚨</div>
              <h3 style={{ margin: "0 0 4px", fontFamily: "var(--font-display)", fontSize: 16, fontWeight: 800 }}>Denunciar {TIPO_LABELS[tipoContenido] ?? "contenido"}</h3>
              <p style={{ margin: 0, fontSize: 12, color: "var(--gfi-text-muted)" }}>
                Ayudás a mantener la comunidad GFI® segura y profesional.
              </p>
            </div>

            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontSize: 10, color: "var(--gfi-text-muted)", fontFamily: "var(--font-display)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 8 }}>
                Motivo *
              </label>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <textarea
                ...
                rows={3}
                style={{ width: "100%", padding: "9px 12px", background: "var(--gfi-border-subtle)", border: "1px solid var(--gfi-border)", borderRadius: 8, color: "#fff", fontSize: 13, resize: "vertical", boxSizing: "border-box" }}
              />
            </div>

            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={onClose} style={{ flex: 1, padding: "10px", background: "var(--gfi-border-subtle)", border: "1px solid var(--gfi-border)", borderRadius: 8, color: "var(--gfi-text-secondary)", cursor: "pointer", fontSize: 13 }}>
                Cancelar
              </button>
              <button
                onClick={enviar}
