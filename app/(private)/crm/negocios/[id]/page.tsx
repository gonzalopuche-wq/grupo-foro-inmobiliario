...
export default function NegocioDetallePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return (
    <div style={{ textAlign: "center", color: "var(--gfi-text-muted)", padding: 60, fontFamily: "Inter,sans-serif" }}>Cargando negocio...</div>
  );
  if (!negocio) return (
    <div style={{ textAlign: "center", color: "var(--gfi-text-muted)", padding: 60, fontFamily: "Inter,sans-serif" }}>
  ...
  return (
    <>
      <style>{`
        ...
        .nd-input { width:100%; padding:8px 10px; background:var(--gfi-border-subtle); border:1px solid var(--gfi-border); border-radius:4px; color:#fff; font-size:13px; font-family:var(--font-body); outline:none; box-sizing:border-box; }
        .nd-input:focus { border-color:rgba(153,0,0,0.5); }
        .nd-select { width:100%; padding:8px 10px; background:var(--gfi-bg-card); border:1px solid var(--gfi-border); border-radius:4px; color:#fff; font-size:13px; font-family:var(--font-body); outline:none; }
        .nd-tab { padding:8px 16px; border-radius:5px; font-family:var(--font-display); font-size:10px; font-weight:700; letter-spacing:0.08em; cursor:pointer; border:1px solid transparent; transition:all 0.15s; }
        .nd-row { display:flex; justify-content:space-between; align-items:center; padding:9px 0; border-bottom:1px solid var(--gfi-border-subtle); }
      ...
      <div style={{ maxWidth: 960, display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <Link href="/crm/negocios" style={{ fontSize: 12, color: "var(--gfi-text-muted)", fontFamily: "Inter,sans-serif", textDecoration: "none" }}>← Negocios</Link>
          <span style={{ color: "rgba(255,255,255,0.15)" }}>·</span>
          <span style={{ fontSize: 12, color: "var(--gfi-text-secondary)", fontFamily: "Inter,sans-serif" }}>{negocio.titulo}</span>
        </div>
        ...
        <div className="nd-card">
          <div style={{ display: "flex", gap: 14, alignItems: "flex-start", flexWrap: "wrap" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontFamily: "var(--font-display)", fontSize: 18, fontWeight: 800, color: "#fff", lineHeight: 1.2, marginBottom: 6 }}>{negocio.titulo}</div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <span style={{ fontSize: 11, padding: "3px 10px", borderRadius: 10, fontFamily: "var(--font-display)", fontWeight: 700, background: `${etapaInfo.color}18`, color: etapaInfo.color, border: `1px solid ${etapaInfo.color}40` }}>
                  {etapaInfo.label}
                </span>
                <span style={{ fontSize: 11, padding: "3px 10px", borderRadius: 10, fontFamily: "var(--font-display)", fontWeight: 700, background: "rgba(255,255,255,0.06)", color: "var(--gfi-text-secondary)", border: "1px solid var(--gfi-border)" }}>
                  {negocio.tipo_operacion}
                </span>
                {negocio.archivado && (
                  <span style={{ fontSize: 11, padding: "3px 10px", borderRadius: 10, fontFamily: "var(--font-display)", fontWeight: 700, background: "rgba(107,114,128,0.15)", color: "#9ca3af", border: "1px solid rgba(107,114,128,0.3)" }}>Archivado</span>
                )}
              </div>
              {negocio.direccion && <div style={{ fontSize: 12, color: "var(--gfi-text-muted)", fontFamily: "Inter,sans-serif", marginTop: 6 }}>📍 {negocio.direccion}</div>}
            </div>
            ...
            <div style={{ display: "flex", gap: 8, flexShrink: 0, flexWrap: "wrap" }}>
              ...
              <Link href="/crm/negocios"
                style={{ padding: "7px 14px", borderRadius: 5, background: "rgba(255,255,255,0.06)", border: "1px solid var(--gfi-border)", color: "var(--gfi-text-secondary)", fontSize: 10, fontFamily: "var(--font-display)", fontWeight: 700, letterSpacing: "0.08em", textDecoration: "none" }}>
                ← Volver
              </Link>
            </div>
          </div>
        ...
        <div className="nd-layout" style={{ display: "flex", gap: 18, alignItems: "flex-start" }}>
          ...
          <div style={{ flex: "0 0 280px", display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            {contacto && (
              <div className="nd-card">
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                  <div style={{ width: 36, height: 36, borderRadius: "50%", background: "rgba(153,0,0,0.1)", border: "1px solid rgba(153,0,0,0.25)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-display)", fontSize: 12, fontWeight: 800, color: "#990000", flexShrink: 0 }}>
                    {contacto.nombre[0]}{contacto.apellido[0]}
                  </div>
                  <div>
                    <Link href={`/crm/contactos/${contacto.id}`} style={{ fontFamily: "var(--font-display)", fontSize: 13, fontWeight: 700, color: "#fff", textDecoration: "none" }}
                      onMouseEnter={e => e.currentTarget.style.color = "#990000"}
                      onMouseLeave={e => e.currentTarget.style.color = "#fff"}>
                      {contacto.nombre} {contacto.apellido}
                    </Link>
                    {contacto.tipo && <div style={{ fontSize: 10, color: "var(--gfi-text-muted)", fontFamily: "Inter,sans-serif", marginTop: 1 }}>{contacto.tipo}</div>}
                  </div>
                </div>
            ...
            <div className="nd-card">
              ...
              {[
                ...
                <div key={row.l} className="nd-row">
                  <span style={{ fontSize: 12, color: "var(--gfi-text-muted)", fontFamily: "Inter,sans-serif" }}>{row.l}</span>
                  <span style={{ fontSize: 12, fontWeight: 700, color: "#fff", fontFamily: "var(--font-display)" }}>{row.v}</span>
                </div>
              ))}
            </div>
                ...
                <div style={{ position: "relative", paddingLeft: 20 }}>
                  ...
                  {timeline.map((t, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 10, position: "relative" }}>
                      ...
                      <div>
                        <div style={{ fontSize: 10, color: t.color, fontFamily: "var(--font-display)", fontWeight: 700, letterSpacing: "0.06em" }}>{t.label}</div>
                        <div style={{ fontSize: 11, color: "var(--gfi-text-secondary)", fontFamily: "Inter,sans-serif" }}>{fmtFecha(t.fecha)}</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="nd-card">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                ...
                <button className="nd-btn" style={{ background: "rgba(255,255,255,0.06)", color: "var(--gfi-text-secondary)", border: "1px solid var(--gfi-border)" }}
              ...
              {editandoNotas ? (
                ...
                <div style={{ fontSize: 12, color: negocio.notas ? "rgba(255,255,255,0.55)" : "var(--gfi-text-dim)", fontFamily: "Inter,sans-serif", lineHeight: 1.5, whiteSpace: "pre-wrap" }}>
          ...
          <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ display: "flex", gap: 6 }}>
              {([
                ...
                <button key={t.key} className="nd-tab"
                  style={{
                    background: tab === t.key ? "rgba(153,0,0,0.12)" : "var(--gfi-border-subtle)",
                    color: tab === t.key ? "#990000" : "var(--gfi-text-muted)",
                    border: `1px solid ${tab === t.key ? "rgba(153,0,0,0.35)" : "var(--gfi-border)"}`,
                  }}
            ...
            {tab === "hitos" && (
              <div className="nd-card">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  <div style={{ fontFamily: "var(--font-display)", fontSize: 13, fontWeight: 700, color: "var(--gfi-text-primary)" }}>Hitos de la operación</div>
                ...
                {hitos.length === 0 ? (
                  <div style={{ textAlign: "center", padding: "32px 0", color: "var(--gfi-text-dim)", fontFamily: "Inter,sans-serif", fontSize: 12 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {hitos.map(h => {
                      ...
                      return (
                        <div key={h.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", background: h.completado ? "rgba(34,197,94,0.05)" : "var(--gfi-bg-secondary)", border: `1px solid ${h.completado ? "rgba(34,197,94,0.15)" : "rgba(255,255,255,0.06)"}`, borderRadius: 6, transition: "all 0.15s" }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontFamily: "var(--font-display)", fontSize: 12, fontWeight: 700, color: h.completado ? "var(--gfi-text-muted)" : "#fff", textDecoration: h.completado ? "line-through" : "none" }}>
                              {info.label}
                            </div>
                            ...
                            {h.notas && <div style={{ fontSize: 11, color: "var(--gfi-text-muted)", fontFamily: "Inter,sans-serif", marginTop: 2 }}>{h.notas}</div>}
                          </div>
                          <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 10, fontFamily: "var(--font-display)", fontWeight: 700, background: `${info.color}15`, color: info.color, border: `1px solid ${info.color}30` }}>
                            {info.label}
                          </span>
                        </div>
                      );
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  <div className="nd-card" style={{ textAlign: "center" }}>
                    <div style={{ fontFamily: "var(--font-display)", fontSize: 10, fontWeight: 700, color: "var(--gfi-text-muted)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 6 }}>Valor de la operación</div>
                    <div style={{ fontFamily: "var(--font-display)", fontSize: 20, fontWeight: 800, color: "#fff" }}>{fmtMon(fin.v, negocio.moneda)}</div>
                  </div>
                  <div className="nd-card" style={{ textAlign: "center", border: "1px solid rgba(34,197,94,0.2)", background: "rgba(34,197,94,0.04)" }}>
                    <div style={{ fontFamily: "var(--font-display)", fontSize: 10, fontWeight: 700, color: "rgba(34,197,94,0.6)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 6 }}>Honorario neto estimado</div>
                    <div style={{ fontFamily: "var(--font-display)", fontSize: 20, fontWeight: 800, color: "#3abab6" }}>{fmtMon(fin.honNeto, negocio.moneda)}</div>
                  </div>
                </div>
                ...
                <div className="nd-card">
                  ...
                  <div className="nd-row">
                    <span style={{ fontSize: 13, color: "var(--gfi-text-secondary)", fontFamily: "Inter,sans-serif" }}>Honorario bruto ({fin.hPct}%)</span>
                    <span style={{ fontFamily: "var(--font-display)", fontSize: 13, fontWeight: 700, color: "#fff" }}>{fmtMon(fin.honBruto, negocio.moneda)}</span>
                  </div>
                  <div className="nd-row">
                    <span style={{ fontSize: 13, color: "var(--gfi-text-secondary)", fontFamily: "Inter,sans-serif" }}>IIBB (5.5%)</span>
                    <span style={{ fontFamily: "var(--font-display)", fontSize: 13, fontWeight: 700, color: "#b80000" }}>− {fmtMon(fin.iibb, negocio.moneda)}</span>
                  </div>
                  <div className="nd-row">
                    <span style={{ fontSize: 13, color: "var(--gfi-text-secondary)", fontFamily: "Inter,sans-serif" }}>IVA (21%)</span>
                    <span style={{ fontFamily: "var(--font-display)", fontSize: 13, fontWeight: 700, color: "#b80000" }}>− {fmtMon(fin.iva, negocio.moneda)}</span>
                  </div>
                  <div className="nd-row" style={{ borderTop: "1px solid var(--gfi-border)", paddingTop: 10, marginTop: 4 }}>
                    <span style={{ fontSize: 13, fontFamily: "var(--font-display)", fontWeight: 700, color: "#3abab6" }}>Honorario neto</span>
                    <span style={{ fontFamily: "var(--font-display)", fontSize: 15, fontWeight: 800, color: "#3abab6" }}>{fmtMon(fin.honNeto, negocio.moneda)}</span>
                  </div>
                  {fin.sPct > 0 && (
                    <div className="nd-row">
                      <span style={{ fontSize: 12, color: "var(--gfi-text-muted)", fontFamily: "Inter,sans-serif" }}>Mi parte ({100 - fin.sPct}% del bruto)</span>
                      <span style={{ fontFamily: "var(--font-display)", fontSize: 12, fontWeight: 700, color: "#d4960c" }}>{fmtMon(fin.miParte, negocio.moneda)}</span>
                    </div>
                  )}
                </div>

                {/* Link a split */}
                <div style={{ padding: "10px 14px", background: "var(--gfi-bg-secondary)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 6, display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 11, color: "var(--gfi-text-muted)", fontFamily: "Inter,sans-serif" }}>Calculá el split exacto con la calculadora de honorarios:</span>
                  <Link href="/calculadoras/split" style={{ fontSize: 11, color: "#990000", fontFamily: "var(--font-display)", fontWeight: 700, textDecoration: "none" }}>Split Honorarios ↗</Link>
                </div>
              </div>
            )}
            ...
            {tab === "postcierrer" && (
              <div className="nd-card">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  <div style={{ fontFamily: "var(--font-display)", fontSize: 13, fontWeight: 700, color: "var(--gfi-text-primary)" }}>Checklist post-cierre</div>
                  <Link href="/crm/post-cierre" style={{ padding: "7px 12px", borderRadius: 5, background: "rgba(255,255,255,0.06)", border: "1px solid var(--gfi-border)", color: "var(--gfi-text-secondary)", fontSize: 10, fontFamily: "var(--font-display)", fontWeight: 700, letterSpacing: "0.08em", textDecoration: "none" }}>
                    Gestionar ↗
                  </Link>
                </div>
                {postCierre.length === 0 ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {postCierre.map(p => (
                      <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: p.completado ? "rgba(34,197,94,0.04)" : "var(--gfi-bg-secondary)", border: `1px solid ${p.completado ? "rgba(34,197,94,0.12)" : "rgba(255,255,255,0.06)"}`, borderRadius: 6 }}>
                        <div style={{ width: 18, height: 18, borderRadius: 3, border: `2px solid ${p.completado ? "#3abab6" : "var(--gfi-text-dim)"}`, background: p.completado ? "#3abab6" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: "#fff", flexShrink: 0 }}>
                          {p.completado ? "✓" : ""}
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 12, fontFamily: "Inter,sans-serif", color: p.completado ? "var(--gfi-text-muted)" : "#fff", textDecoration: p.completado ? "line-through" : "none" }}>{p.titulo}</div>
                          {p.fecha_limite && <div style={{ fontSize: 10, color: "var(--gfi-text-muted)", fontFamily: "Inter,sans-serif", marginTop: 2 }}>Límite: {fmtFecha(p.fecha_limite)}</div>}
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      {modalHito && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: "var(--gfi-bg-secondary)", border: "1px solid var(--gfi-border)", borderRadius: 10, padding: 24, width: "100%", maxWidth: 420 }}>
            <div style={{ fontFamily: "var(--font-display)", fontSize: 14, fontWeight: 800, color: "#fff", marginBottom: 18 }}>Agregar hito</div>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
              <button className="nd-btn" style={{ background: "rgba(255,255,255,0.06)", color: "var(--gfi-text-secondary)", border: "1px solid var(--gfi-border)" }} onClick={() => setModalHito(null)}>Cancelar</button>
