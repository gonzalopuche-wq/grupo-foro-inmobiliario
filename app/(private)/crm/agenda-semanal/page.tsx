...
export default function AgendaSemanal() {
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#0a0a0a", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ color: "var(--gfi-text-muted)", fontFamily: "Inter,sans-serif" }}>Cargando agenda…</div>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0a", color: "#fff", fontFamily: "Inter,sans-serif", padding: "28px 24px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
        ...
        <h1 style={{ fontFamily: "var(--font-display)", fontSize: 18, fontWeight: 800, margin: 0 }}>Agenda Semanal</h1>
      </div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 20, alignItems: "center", flexWrap: "wrap" }}>
        ...
        {[
          ...
          <button key={t.id} onClick={() => toggleTipo(t.id)}
            style={{ background: filtroTipos.has(t.id) ? t.color + "22" : "var(--gfi-border-subtle)", border: `1px solid ${filtroTipos.has(t.id) ? t.color + "44" : "var(--gfi-border)"}`, borderRadius: 6, color: filtroTipos.has(t.id) ? t.color : "var(--gfi-text-muted)", fontSize: 11, padding: "5px 12px", cursor: "pointer", fontFamily: "var(--font-display)", fontWeight: 700 }}>
            {t.icono} {t.label}
          </button>
        ))}
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <button onClick={() => setSemanaOffset(v => v - 1)}
            style={{ background: "var(--gfi-bg-secondary)", border: "1px solid var(--gfi-border)", borderRadius: 6, color: "var(--gfi-text-secondary)", fontSize: 14, padding: "5px 12px", cursor: "pointer" }}>←</button>
          <button onClick={() => setSemanaOffset(0)}
            style={{ background: semanaOffset === 0 ? "rgba(153,0,0,0.15)" : "var(--gfi-bg-secondary)", border: `1px solid ${semanaOffset === 0 ? "rgba(153,0,0,0.3)" : "var(--gfi-border)"}`, borderRadius: 6, color: semanaOffset === 0 ? "#990000" : "var(--gfi-text-secondary)", fontSize: 11, padding: "5px 12px", cursor: "pointer", fontFamily: "var(--font-display)", fontWeight: 700 }}>
            Hoy
          </button>
          <button onClick={() => setSemanaOffset(v => v + 1)}
            style={{ background: "var(--gfi-bg-secondary)", border: "1px solid var(--gfi-border)", borderRadius: 6, color: "var(--gfi-text-secondary)", fontSize: 14, padding: "5px 12px", cursor: "pointer" }}>→</button>
          ...
          {(["semana", "lista"] as const).map(v => (
            <button key={v} onClick={() => setVistaMode(v)}
              style={{ background: vistaMode === v ? "rgba(153,0,0,0.15)" : "var(--gfi-border-subtle)", border: `1px solid ${vistaMode === v ? "rgba(153,0,0,0.3)" : "var(--gfi-border)"}`, borderRadius: 6, color: vistaMode === v ? "#990000" : "var(--gfi-text-muted)", fontSize: 11, padding: "5px 10px", cursor: "pointer", fontFamily: "var(--font-display)", fontWeight: 700 }}>
      ...
      {/* Título de semana */}
      <div style={{ fontFamily: "var(--font-display)", fontSize: 13, fontWeight: 700, color: "var(--gfi-text-secondary)", marginBottom: 14 }}>
      ...
      {vistaMode === "semana" ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 8 }}>
          {diasSemanaAjustados.map(dia => {
            ...
            return (
              <div key={diaStr} style={{ background: esHoy ? "rgba(153,0,0,0.06)" : "var(--gfi-bg-primary)", border: `1px solid ${esHoy ? "rgba(153,0,0,0.3)" : "var(--gfi-border-subtle)"}`, borderRadius: 10, padding: "12px 10px", minHeight: 160 }}>
                {/* Cabecera del día */}
                <div style={{ marginBottom: 10, textAlign: "center" }}>
                  <div style={{ fontSize: 9, fontFamily: "var(--font-display)", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: esHoy ? "#990000" : "var(--gfi-text-muted)" }}>
                    {DIAS_SEMANA[dia.getDay()]}
                  </div>
                  <div style={{ fontFamily: "var(--font-display)", fontSize: 20, fontWeight: 800, color: esHoy ? "#990000" : esPassado ? "var(--gfi-text-dim)" : "rgba(255,255,255,0.85)", lineHeight: 1.2 }}>
                    {dia.getDate()}
                  </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  {eventosDelDia.map(ev => (
                    <div key={ev.id} style={{ background: esPassado && !ev.completado ? "rgba(239,68,68,0.08)" : ev.bgColor, borderLeft: `2px solid ${esPassado && !ev.completado ? "#b80000" : ev.color}`, borderRadius: "0 4px 4px 0", padding: "4px 6px" }}>
                      <div style={{ fontSize: 9, color: esPassado && !ev.completado ? "#b80000" : ev.color, fontFamily: "var(--font-display)", fontWeight: 700 }}>{ev.icono}</div>
                      <div style={{ fontSize: 9, color: "var(--gfi-text-primary)", lineHeight: 1.3, marginTop: 1, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>
                        {ev.titulo}
                      </div>
                      {ev.link && (
                        <Link href={ev.link} style={{ fontSize: 8, color: "var(--gfi-text-muted)", textDecoration: "none" }}>↗</Link>
                      )}
                    </div>
                  ))}
                  {eventosDelDia.length === 0 && (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {eventos.filter(e => e.fecha < hoy).length > 0 && (
            <div style={{ ...cardStyle, borderColor: "rgba(239,68,68,0.25)" }}>
              <div style={{ fontFamily: "var(--font-display)", fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#b80000", marginBottom: 10 }}>⚠ Vencidos</div>
              {eventos.filter(e => e.fecha < hoy).map(ev => (
                <div key={ev.id} style={{ display: "flex", gap: 10, alignItems: "center", padding: "7px 0", borderBottom: "1px solid var(--gfi-border-subtle)" }}>
                  <span style={{ fontSize: 12 }}>{ev.icono}</span>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 12, color: "var(--gfi-text-primary)" }}>{ev.titulo}</div>
                    <div style={{ fontSize: 10, color: "#b80000" }}>Venció {ev.fecha}</div>
                  </div>
                  {ev.link && <Link href={ev.link} style={{ fontSize: 10, color: "var(--gfi-text-muted)", textDecoration: "none" }}>↗</Link>}
                </div>
              ))}
            </div>
          )}
              ...
              return (
                <div key={fecha} style={{ ...cardStyle, borderColor: esHoyDate ? "rgba(153,0,0,0.3)" : undefined }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                    <div style={{ fontFamily: "var(--font-display)", fontSize: 22, fontWeight: 800, color: esHoyDate ? "#990000" : "var(--gfi-text-primary)", lineHeight: 1 }}>{dObj.getDate()}</div>
                    <div>
                      <div style={{ fontFamily: "var(--font-display)", fontSize: 11, fontWeight: 700, color: esHoyDate ? "#990000" : "var(--gfi-text-muted)" }}>{DIAS_SEMANA[dObj.getDay()]}</div>
                      <div style={{ fontSize: 10, color: "var(--gfi-text-muted)" }}>{MESES[dObj.getMonth()]} {dObj.getFullYear()}</div>
                    </div>
                    {esHoyDate && <span style={{ background: "#990000", color: "#fff", fontSize: 9, fontFamily: "var(--font-display)", fontWeight: 700, padding: "2px 7px", borderRadius: 4 }}>HOY</span>}
                    <span style={{ marginLeft: "auto", background: "var(--gfi-border-subtle)", color: "var(--gfi-text-muted)", fontSize: 10, padding: "2px 8px", borderRadius: 4 }}>{evs.length}</span>
                  </div>
                  {evs.map(ev => (
                    <div key={ev.id} style={{ display: "flex", gap: 10, alignItems: "center", padding: "8px 10px", borderRadius: 6, background: ev.bgColor, borderLeft: `3px solid ${ev.color}`, marginBottom: 6 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 9, color: ev.color, fontFamily: "var(--font-display)", fontWeight: 700, textTransform: "uppercase", marginTop: 2 }}>{ev.tipo}</div>
                      </div>
                      {ev.link && <Link href={ev.link} style={{ fontSize: 10, color: "var(--gfi-text-muted)", textDecoration: "none", whiteSpace: "nowrap" }}>Ver ↗</Link>}
                    </div>
                  ))}
                </div>
              );
          ...
          {eventosLista.length === 0 && (
            <div style={{ textAlign: "center", padding: "40px 0", color: "var(--gfi-text-muted)" }}>
              <div style={{ fontSize: 32, marginBottom: 12 }}>📅</div>
              <div style={{ fontFamily: "var(--font-display)", fontSize: 14 }}>Sin eventos en los próximos 30 días</div>
            </div>
          )}
        </div>
      )}
