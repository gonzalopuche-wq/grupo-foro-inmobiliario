  ...
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
        <span style={{ fontSize: 12, color: "var(--gfi-text-primary)" }}>{label}</span>
        <span style={{ fontSize: 12, fontWeight: 600, color }}>{count} <span style={{ color: "var(--gfi-text-muted)", fontWeight: 400 }}>({pct.toFixed(0)}%)</span></span>
      </div>
...
export default function EstadisticasPage() {
  ...
  const card = (titulo: string, valor: string | number, sub?: string, color = "#fff") => (
    <div style={{ background: "var(--gfi-bg-card)", border: "1px solid var(--gfi-border)", borderRadius: 10, padding: "20px 22px" }}>
      <div style={{ fontSize: 11, fontFamily: "var(--font-display)", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--gfi-text-muted)", marginBottom: 10 }}>{titulo}</div>
      <div style={{ fontSize: 32, fontWeight: 800, fontFamily: "var(--font-display)", color }}>{valor}</div>
      {sub && <div style={{ fontSize: 11, color: "var(--gfi-text-muted)", marginTop: 4 }}>{sub}</div>}
    </div>
  );
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .est-topbar { display: flex; align-items: center; gap: 12px; padding: 14px 20px; border-bottom: 1px solid var(--gfi-border-subtle); }
        .est-back { font-family: Montserrat,sans-serif; font-size: 10px; font-weight: 700; letter-spacing: 0.1em; color: var(--gfi-text-muted); text-decoration: none; text-transform: uppercase; }
        ...
        .est-section-title { font-family: Montserrat,sans-serif; font-size: 11px; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; color: var(--gfi-text-muted); margin-bottom: 14px; }
        .est-panel { background: var(--gfi-bg-card); border: 1px solid var(--gfi-border); border-radius: 10px; padding: 20px 22px; }
        ...
        .est-reciente { display: flex; align-items: center; gap: 12px; padding: 10px 0; border-bottom: 1px solid var(--gfi-border-subtle); }
      ...
      <div className="est-wrap">
        ...
        <div className="est-content">
          {loading || !stats ? (
            <div style={{ color: "var(--gfi-text-muted)", padding: 40, textAlign: "center" }}>Cargando estadísticas...</div>
          ) : (
            <>
              {/* KPIs */}
                ...
                <div className="est-section">
                  ...
                  <div className="est-panel" style={{ display: "flex", gap: 32, flexWrap: "wrap" }}>
                    {Object.entries(stats.valorTotal).map(([moneda, total]) => (
                      <div key={moneda}>
                        <div style={{ fontSize: 11, color: "var(--gfi-text-muted)", fontFamily: "var(--font-display)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>{moneda}</div>
                        <div style={{ fontSize: 28, fontWeight: 800, fontFamily: "var(--font-display)", color: "#3abab6" }}>{formatMoney(total, moneda)}</div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="est-section">
                ...
                <div className="est-panel-2col">
                  <div className="est-panel">
                    <div style={{ fontSize: 11, fontFamily: "var(--font-display)", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--gfi-text-muted)", marginBottom: 14 }}>Por operación</div>
                    {Object.entries(stats.porOperacion).map(([op, n]) => (
                      <StatBar key={op} label={op} count={n} total={stats.total} color={OP_COLOR[op] ?? "#888"} />
                    ))}
                  </div>
                  <div className="est-panel">
                    <div style={{ fontSize: 11, fontFamily: "var(--font-display)", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--gfi-text-muted)", marginBottom: 14 }}>Por tipo</div>
              ...
              <div className="est-section">
                ...
                <div className="est-panel">
                  <div style={{ display: "flex", gap: 32, flexWrap: "wrap" }}>
                    <div>
                      <div style={{ fontSize: 11, color: "var(--gfi-text-muted)", fontFamily: "var(--font-display)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>Tokko Broker</div>
                      <div style={{ fontSize: 28, fontWeight: 800, fontFamily: "var(--font-display)", color: stats.syncStatus.tokko > 0 ? "#3abab6" : "var(--gfi-text-muted)" }}>
                        {stats.syncStatus.tokko}
                      </div>
                      <div style={{ fontSize: 11, color: "var(--gfi-text-muted)" }}>sincronizadas</div>
                    </div>
                    <div>
                      <div style={{ fontSize: 11, color: "var(--gfi-text-muted)", fontFamily: "var(--font-display)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>KiteProp</div>
                      <div style={{ fontSize: 28, fontWeight: 800, fontFamily: "var(--font-display)", color: stats.syncStatus.kiteprop > 0 ? "#3abab6" : "var(--gfi-text-muted)" }}>
                        {stats.syncStatus.kiteprop}
                      </div>
                      <div style={{ fontSize: 11, color: "var(--gfi-text-muted)" }}>sincronizadas</div>
                    </div>
                    <div>
                      <div style={{ fontSize: 11, color: "var(--gfi-text-muted)", fontFamily: "var(--font-display)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>Sin sincronizar</div>
                      <div style={{ fontSize: 28, fontWeight: 800, fontFamily: "var(--font-display)", color: stats.syncStatus.sinSync > 0 ? "#d4960c" : "#3abab6" }}>
                        {stats.syncStatus.sinSync}
                      </div>
                      <div style={{ fontSize: 11, color: "var(--gfi-text-muted)" }}>solo en GFI</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {stats.leads.total > 0 && (
                <div className="est-section">
                  ...
                  <div className="est-panel-2col">
                    <div className="est-panel">
                      <div style={{ fontSize: 11, fontFamily: "var(--font-display)", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--gfi-text-muted)", marginBottom: 14 }}>Por estado</div>
                    ...
                    <div className="est-panel">
                      <div style={{ fontSize: 11, fontFamily: "var(--font-display)", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--gfi-text-muted)", marginBottom: 14 }}>Por origen</div>
                ...
                <div className="est-section">
                  ...
                  <div className="est-panel" style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
                    {Object.entries(stats.visitas.porEstado).map(([est, n]) => (
                      <div key={est}>
                        <div style={{ fontSize: 11, color: "var(--gfi-text-muted)", fontFamily: "var(--font-display)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 4 }}>{est.replace(/_/g," ")}</div>
                        <div style={{ fontSize: 24, fontWeight: 800, fontFamily: "var(--font-display)", color: est === "realizada" ? "#3abab6" : est === "cancelada" ? "#6b7280" : "#fff" }}>{n}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="est-section">
                  ...
                  <div className="est-panel">
                    <div style={{ display:"grid", gridTemplateColumns:"1fr auto auto", gap:"8px 16px", alignItems:"center" }}>
                      <div style={{ fontSize:10, fontFamily:"Montserrat,sans-serif", fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", color:"var(--gfi-text-dim)" }}>Agente</div>
                      <div style={{ fontSize:10, fontFamily:"Montserrat,sans-serif", fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", color:"var(--gfi-text-dim)", textAlign:"right" }}>Leads</div>
                      <div style={{ fontSize:10, fontFamily:"Montserrat,sans-serif", fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", color:"var(--gfi-text-dim)", textAlign:"right" }}>Visitas</div>
                      {agentStats.map(a => (<>
                        <div key={a.nombre} style={{ fontSize:13, color:"rgba(255,255,255,0.8)", paddingTop:6, borderTop:"1px solid var(--gfi-border-subtle)" }}>{a.nombre}</div>
                        <div style={{ fontSize:18, fontWeight:800, fontFamily:"Montserrat,sans-serif", color: a.leads > 0 ? "#4ab8d8" : "var(--gfi-text-dim)", textAlign:"right", borderTop:"1px solid var(--gfi-border-subtle)", paddingTop:6 }}>{a.leads}</div>
                        <div style={{ fontSize:18, fontWeight:800, fontFamily:"Montserrat,sans-serif", color: a.visitas > 0 ? "#3abab6" : "var(--gfi-text-dim)", textAlign:"right", borderTop:"1px solid var(--gfi-border-subtle)", paddingTop:6 }}>{a.visitas}</div>
                      </>))}
                    </div>
                  </div>
                </div>
                  ...
                  <div className="est-panel">
                    {stats.topVistas.map((p, i) => (
                      <div key={p.id} className="est-reciente">
                        <div style={{ width: 20, fontSize: 11, fontFamily: "var(--font-display)", fontWeight: 800, color: i === 0 ? "#d4960c" : "var(--gfi-text-muted)", flexShrink: 0 }}>#{i + 1}</div>
                        ...
                        <div style={{ fontSize: 13, fontWeight: 700, fontFamily: "var(--font-display)", color: "#a78bfa", minWidth: 48, textAlign: "right" }}>
                          👁 {p.vistas}
                        </div>
                      </div>
                    ))}
                ...
                <div className="est-section">
                  ...
                  <div className="est-panel">
                    <div style={{ display: "flex", gap: 32, flexWrap: "wrap", marginBottom: 20 }}>
                      <div>
                        <div style={{ fontSize: 11, color: "var(--gfi-text-muted)", fontFamily: "var(--font-display)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>Total</div>
                        <div style={{ fontSize: 28, fontWeight: 800, fontFamily: "var(--font-display)", color: "#fff" }}>{stats.webLeads.total}</div>
                      </div>
                      <div>
                        <div style={{ fontSize: 11, color: "var(--gfi-text-muted)", fontFamily: "var(--font-display)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>Este mes</div>
                        <div style={{ fontSize: 28, fontWeight: 800, fontFamily: "var(--font-display)", color: "#3abab6" }}>{stats.webLeads.esteMes}</div>
                      </div>
                      {stats.webLeads.noLeidos > 0 && (
                        <div>
                          <div style={{ fontSize: 11, color: "var(--gfi-text-muted)", fontFamily: "var(--font-display)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>Sin leer</div>
                          <div style={{ fontSize: 28, fontWeight: 800, fontFamily: "var(--font-display)", color: "#b80000" }}>{stats.webLeads.noLeidos}</div>
                        </div>
                      )}
                    </div>
                    ...
                    <div style={{ marginTop: 14 }}>
                      <Link href="/mi-web/leads" style={{ fontSize: 11, color: "#4ab8d8", fontFamily: "var(--font-display)", fontWeight: 700, textDecoration: "none" }}>
                        Ver todos los leads →
                      </Link>
                    </div>
                  </div>
