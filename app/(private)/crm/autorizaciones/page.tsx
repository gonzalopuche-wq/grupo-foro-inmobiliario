...
const ESTADO_COLORS: Record<string, { bg: string; color: string; label: string }> = {
  ...
  cancelada: { bg: "rgba(255,255,255,0.06)", color: "var(--gfi-text-muted)", label: "Cancelada" },
};
...
export default function AutorizacionesPage() {
  ...
  if (loading) return <div style={{ color: "var(--gfi-text-muted)", padding: 40, textAlign: "center" }}>Cargando...</div>;

  return (
    <>
      <style>{`
        .aut-table { width: 100%; border-collapse: collapse; font-family: Inter,sans-serif; }
        .aut-table th { font-family: Montserrat,sans-serif; font-size: 9px; font-weight: 700; letter-spacing: 0.14em; text-transform: uppercase; color: var(--gfi-text-muted); padding: 8px 12px; text-align: left; border-bottom: 1px solid var(--gfi-border-subtle); }
        .aut-table th.sortable { cursor: pointer; user-select: none; }
        .aut-table th.sortable:hover { color: var(--gfi-text-secondary); }
        .aut-table th.sort-activo { color: #990000 !important; }
        .aut-table td { padding: 12px; border-bottom: 1px solid var(--gfi-border-subtle); font-size: 13px; color: rgba(255,255,255,0.75); vertical-align: middle; }
        .aut-table tr:hover td { background: var(--gfi-bg-secondary); cursor: pointer; }
        ...
        .modal-box { background: #111; border: 1px solid var(--gfi-border); border-radius: 12px; padding: 28px; width: 100%; max-width: 540px; max-height: 90vh; overflow-y: auto; }
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <div>
          <div style={{ fontFamily: "var(--font-display)", fontSize: 18, fontWeight: 800, color: "#fff", marginBottom: 4 }}>
        ...
        <button onClick={() => { setForm({ ...EMPTY }); setModal("nuevo"); }} style={{
          ...
          color: "#fff", cursor: "pointer", fontFamily: "var(--font-display)", fontSize: 10,
              ...
              {ordenados.map(item => {
                ...
                return (
                  <tr key={item.id} onClick={() => { setVerItem(item); setModal("ver"); }}>
                    ...
                    <td><span style={{ fontSize: 11, color: "var(--gfi-text-secondary)" }}>{TIPO_OP[item.tipo_operacion] ?? item.tipo_operacion}</span></td>
                    ...
                    <td>
                      {alerta ? (
                        ...
                        <span style={{ color: "var(--gfi-text-muted)", fontSize: 12 }}>{dias}d</span>
                      ) : "—"}
                    </td>
                  </tr>
                );
        ...
        <div className="modal-over" onClick={e => { if (e.target === e.currentTarget) setModal(null); }}>
          <div className="modal-box">
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
              <button onClick={() => setModal(null)} style={{ padding: "10px 20px", background: "transparent", border: "1px solid rgba(255,255,255,0.14)", borderRadius: 4, color: "rgba(255,255,255,0.45)", cursor: "pointer", fontSize: 11, fontFamily: "var(--font-display)", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase" }}>Cancelar</button>
              <button onClick={guardar} disabled={guardando} style={{ padding: "10px 24px", background: "#990000", border: "none", borderRadius: 4, color: "#fff", cursor: guardando ? "not-allowed" : "pointer", opacity: guardando ? 0.6 : 1, fontSize: 11, fontFamily: "var(--font-display)", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase" }}>
      ...
      {modal === "ver" && verItem && (() => {
        ...
        const row = (label: string, val: string | null | undefined) => val ? (
          <div style={{ marginBottom: 10 }}>
            <div style={{ fontSize: 9, fontFamily: "var(--font-display)", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--gfi-text-muted)", marginBottom: 3 }}>{label}</div>
          ...
          <div className="modal-over" onClick={e => { if (e.target === e.currentTarget) { setModal(null); setVerItem(null); } }}>
            <div className="modal-box">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
                ...
                <button onClick={() => { setModal(null); setVerItem(null); }} style={{ background: "none", border: "none", color: "var(--gfi-text-muted)", cursor: "pointer", fontSize: 18 }}>✕</button>
              </div>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
                ...
                <span style={{ fontSize: 10, color: "var(--gfi-text-muted)", padding: "3px 8px", background: "var(--gfi-border-subtle)", borderRadius: 10 }}>{TIPO_OP[verItem.tipo_operacion]}</span>
              </div>
              ...
              {verItem.estado === "vigente" && (
                <div style={{ display: "flex", gap: 8, marginTop: 20, flexWrap: "wrap" }}>
                  ...
                  <button onClick={() => cambiarEstado(verItem.id, "cancelada")} style={{ padding: "8px 16px", background: "var(--gfi-border-subtle)", border: "1px solid var(--gfi-border)", borderRadius: 4, color: "var(--gfi-text-muted)", cursor: "pointer", fontSize: 11, fontWeight: 700 }}>Cancelar</button>
                </div>
              )}

              <div style={{ marginTop: 16, paddingTop: 12, borderTop: "1px solid var(--gfi-border-subtle)", display: "flex", justifyContent: "flex-end" }}>
                <button onClick={() => eliminar(verItem.id)} style={{ padding: "6px 14px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 4, color: "#b80000", cursor: "pointer", fontSize: 10, fontWeight: 700, fontFamily: "var(--font-display)", letterSpacing: "0.08em" }}>Eliminar</button>
              </div>
            </div>
          </div>
        );
