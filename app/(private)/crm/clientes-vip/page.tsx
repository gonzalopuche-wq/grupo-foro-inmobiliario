...
function tipoColor(t: ClienteVIP["tipoVIP"]) {
  return t === "platinum" ? "#e5e7eb" : t === "gold" ? "#d4960c" : t === "silver" ? "#9ca3af" : "var(--gfi-text-muted)";
}
...
export default function ClientesVIP() {
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#0a0a0a", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <span style={{ color: "var(--gfi-text-muted)", fontFamily: "var(--font-body)" }}>Cargando clientes VIP...</span>
    </div>
  );
    ...
    <div style={{ minHeight: "100vh", background: "#0a0a0a", color: "#fff", fontFamily: "var(--font-body)" }}>
      ...
      <div style={{ background: "var(--gfi-bg-secondary)", borderBottom: "1px solid rgba(255,255,255,0.06)", padding: "16px 24px", display: "flex", alignItems: "center", gap: 16 }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <input type="number" value={tcDolar} onChange={e => setTcDolar(+e.target.value)} style={{ width: 80, background: "var(--gfi-border-subtle)", border: "1px solid var(--gfi-border)", borderRadius: 6, color: "#fff", padding: "4px 8px", fontSize: 11 }} />
          <span style={{ fontSize: 11, color: "var(--gfi-text-muted)" }}>Hon.%:</span>
          <input type="number" step="0.5" value={honorariosPct} onChange={e => setHonorariosPct(+e.target.value)} style={{ width: 55, background: "var(--gfi-border-subtle)", border: "1px solid var(--gfi-border)", borderRadius: 6, color: "#fff", padding: "4px 8px", fontSize: 11 }} />
        </div>
      </div>
      ...
      <div style={{ padding: "24px", maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 16, alignItems: "center", flexWrap: "wrap" }}>
          <input type="text" placeholder="Buscar cliente..." value={busqueda} onChange={e => setBusqueda(e.target.value)} style={{ flex: 1, minWidth: 160, background: "var(--gfi-border-subtle)", border: "1px solid var(--gfi-border)", borderRadius: 8, color: "#fff", padding: "7px 12px", fontFamily: "var(--font-body)", fontSize: 12 }} />
          <div style={{ display: "flex", gap: 6 }}>
            {(["todos","platinum","gold","silver"] as const).map(t => (
              <button key={t} onClick={() => setFiltroTipo(t)} style={{ padding: "5px 12px", borderRadius: 20, border: `1px solid ${filtroTipo === t ? "rgba(153,0,0,0.5)" : "var(--gfi-border)"}`, background: filtroTipo === t ? "rgba(153,0,0,0.12)" : "transparent", color: filtroTipo === t ? "#990000" : "var(--gfi-text-muted)", fontSize: 10, fontFamily: "var(--font-display)", fontWeight: 700, cursor: "pointer", textTransform: "capitalize" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: seleccionado ? "1fr 320px" : "1fr", gap: 20 }}>
              ...
              <thead>
                <tr style={{ background: "var(--gfi-bg-card)" }}>
                  {["Tier", "Cliente", "Operaciones", "Valor total", "Honorarios", "Interacciones", "P. Referido", "Score"].map(h => (
                    <th key={h} style={{ padding: "10px 14px", textAlign: h === "Tier" || h === "Cliente" ? "left" : "center", fontSize: 9, color: "var(--gfi-text-muted)", fontFamily: "var(--font-display)", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", borderBottom: "1px solid var(--gfi-border-subtle)" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtrados.length === 0 ? (
                  <tr><td colSpan={8} style={{ padding: 40, textAlign: "center", color: "var(--gfi-text-dim)", fontSize: 13 }}>Sin clientes VIP aún</td></tr>
                ) : filtrados.map((c, idx) => (
                  <tr key={c.id} onClick={() => setSeleccionado(seleccionado?.id === c.id ? null : c)} style={{ cursor: "pointer", background: seleccionado?.id === c.id ? "rgba(153,0,0,0.06)" : idx % 2 === 0 ? "transparent" : "rgba(255,255,255,0.01)", borderLeft: seleccionado?.id === c.id ? "2px solid #990000" : "2px solid transparent" }}>
                    ...
                    <td style={{ padding: "10px 14px" }}>
                      ...
                      <div style={{ fontSize: 10, color: "var(--gfi-text-muted)" }}>{c.tipo ?? "—"}{c.zona_interes ? ` · ${c.zona_interes}` : ""}</div>
                    </td>
                    <td style={{ padding: "10px 14px", textAlign: "center" }}>
                      <span style={{ fontSize: 13, fontWeight: 700 }}>{c.negociosCerrados}</span>
                      <span style={{ fontSize: 10, color: "var(--gfi-text-muted)" }}>/{c.negociosTotal}</span>
                    </td>
                    ...
                    <td style={{ padding: "10px 14px", textAlign: "center" }}>
                      <span style={{ fontSize: 13, fontWeight: 800, fontFamily: "var(--font-display)", color: tipoColor(c.tipoVIP) }}>{c.scoreVIP.toFixed(0)}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          {/* Panel detalle */}
          {seleccionado && (
            <div style={{ background: "var(--gfi-bg-secondary)", border: "1px solid rgba(153,0,0,0.2)", borderRadius: 12, padding: 20, alignSelf: "start" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                <div>
                  <div style={{ fontSize: 22, marginBottom: 4 }}>{tipoIcon(seleccionado.tipoVIP)}</div>
                  <h3 style={{ margin: 0, fontSize: 16, fontFamily: "var(--font-display)", fontWeight: 800 }}>{seleccionado.nombre} {seleccionado.apellido}</h3>
                  <p style={{ margin: "2px 0 0 0", fontSize: 11, color: tipoColor(seleccionado.tipoVIP), fontFamily: "var(--font-display)", fontWeight: 700, textTransform: "uppercase" }}>{seleccionado.tipoVIP}</p>
                </div>
                <button onClick={() => setSeleccionado(null)} style={{ background: "none", border: "none", color: "var(--gfi-text-muted)", cursor: "pointer", fontSize: 16 }}>✕</button>
              </div>

              {seleccionado.email && <p style={{ margin: "0 0 4px 0", fontSize: 11, color: "var(--gfi-text-muted)" }}>✉ {seleccionado.email}</p>}
              ...
              {seleccionado.zona_interes && <p style={{ margin: "0 0 12px 0", fontSize: 11, color: "var(--gfi-text-muted)" }}>📍 {seleccionado.zona_interes}</p>}
              ...
              {[
                ...
                <div key={row.label} style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: "1px solid var(--gfi-border-subtle)" }}>
                  <span style={{ fontSize: 11, color: "var(--gfi-text-muted)" }}>{row.label}</span>
              ...
              {seleccionado.etiquetas && seleccionado.etiquetas.length > 0 && (
                <div style={{ marginTop: 12, display: "flex", flexWrap: "wrap", gap: 5 }}>
                  {seleccionado.etiquetas.map(e => (
                    <span key={e} style={{ fontSize: 9, padding: "2px 8px", borderRadius: 10, background: "rgba(255,255,255,0.06)", color: "var(--gfi-text-secondary)", fontFamily: "var(--font-display)", fontWeight: 700 }}>{e}</span>
                  ))}
                </div>
              )}
              ...
              {seleccionado.probabilidadReferido >= 60 && (
                <div style={{ marginTop: 14, padding: 10, borderRadius: 8, background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.2)" }}>
                  <p style={{ margin: 0, fontSize: 11, color: "#3abab6", fontFamily: "var(--font-display)", fontWeight: 700 }}>
              ...
              {seleccionado.notas && (
                <div style={{ marginTop: 12 }}>
                  <p style={{ margin: "0 0 4px 0", fontSize: 9, color: "var(--gfi-text-muted)", fontFamily: "var(--font-display)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em" }}>Notas</p>
                  <p style={{ margin: 0, fontSize: 11, color: "var(--gfi-text-secondary)", lineHeight: 1.5 }}>{seleccionado.notas}</p>
                </div>
              )}
            </div>
          )}
