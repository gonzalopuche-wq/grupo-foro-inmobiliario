...
function GraficoBarras({ data }: { data: SemanaData[] }) {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ display: "block", overflow: "visible" }}>
      ...
      <line x1={padL} y1={H - padB} x2={W} y2={H - padB} stroke="rgba(255,255,255,0.1)" strokeWidth={1} />
      ...
      {[0.25, 0.5, 0.75, 1].map(f => {
        ...
        return (
          <g key={f}>
            <line x1={padL} y1={y} x2={W} y2={y} stroke="rgba(255,255,255,0.05)" strokeWidth={1} strokeDasharray="4,4" />
            <text x={padL - 4} y={y + 4} textAnchor="end" fill="rgba(255,255,255,0.25)" fontSize={9} fontFamily="Inter,sans-serif">{val}</text>
          </g>
        );
      })}
      ...
      <text x={W - 116} y={padT + 9} fill="rgba(255,255,255,0.4)" fontSize={9} fontFamily="Inter,sans-serif">Programadas</text>
      <rect x={W - 130} y={padT + 16} width={10} height={10} fill="rgba(34,197,94,0.6)" rx={2} />
      <text x={W - 116} y={padT + 25} fill="rgba(255,255,255,0.4)" fontSize={9} fontFamily="Inter,sans-serif">Realizadas</text>
    </svg>
  );
}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                        ...
                        {tasDelDia.length === 0 && (
                          <div style={{ textAlign: "center", fontSize: 9, color: "rgba(255,255,255,0.08)", paddingTop: 8 }}>—</div>
                        )}
                      </div>
                    </div>
                  );
              ...
              {cardExpandida && (() => {
                ...
                return (
                  <div style={{
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10 }}>
                      <div>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                          <span style={{ fontFamily: "Montserrat,sans-serif", fontSize: 14, fontWeight: 800 }}>{tas.contacto_nombre}</span>
                    ...
                    {tas.notas && (
                      <div style={{ marginTop: 10, padding: "8px 10px", background: "rgba(255,255,255,0.03)", borderRadius: 5, fontSize: 12, color: "rgba(255,255,255,0.5)" }}>
                        {tas.notas}
                      </div>
                    )}
                    <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
                      ...
                      {(["programada", "realizada", "cancelada", "sin_respuesta"] as EstadoTasacion[]).map(e => (
                        <button
                          ...
                          style={{
                            padding: "6px 12px",
                            background: tas.estado === e ? ESTADO_CONFIG[e].bg : "rgba(255,255,255,0.04)",
                            color: tas.estado === e ? ESTADO_CONFIG[e].color : "rgba(255,255,255,0.35)",
                            border: `1px solid ${tas.estado === e ? ESTADO_CONFIG[e].color + "50" : "rgba(255,255,255,0.08)"}`,
                            borderRadius: 5,
                            fontSize: 10,
                            fontFamily: "Montserrat,sans-serif",
                            fontWeight: 700,
                  ...
                  <tbody>
                    {tasacionesFiltradas.map(t => (
                      <tr key={t.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                        <td style={{ padding: "10px 10px", whiteSpace: "nowrap", color: "rgba(255,255,255,0.7)" }}>
                          <div style={{ fontFamily: "Montserrat,sans-serif", fontWeight: 700, fontSize: 11 }}>{t.hora_programada}</div>
                          <div style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", marginTop: 1 }}>{fmtFechaCorta(t.fecha_programada)}</div>
                        </td>
                        <td style={{ padding: "10px 10px" }}>
                          <div style={{ color: "#fff", fontWeight: 600 }}>{t.contacto_nombre}</div>
                          {t.contacto_telefono && (
                            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", marginTop: 1 }}>{t.contacto_telefono}</div>
                          )}
                        </td>
                        <td style={{ padding: "10px 10px", maxWidth: 160 }}>
                          <div style={{ color: "rgba(255,255,255,0.7)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.direccion}</div>
                          {t.barrio && <div style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", marginTop: 1 }}>{t.barrio}</div>}
                        </td>
                  ...
                  {TIPOS_PROPIEDAD.map(tipo => {
                    ...
                    return (
                      <div key={tipo} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <span style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", fontFamily: "Montserrat,sans-serif", width: 100, flexShrink: 0 }}>
                          {TIPO_LABEL[tipo]}
                        </span>
                        ...
                        <span style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", width: 40, textAlign: "right", fontFamily: "Inter,sans-serif" }}>
                          {count} ({pct}%)
                        </span>
                      </div>
                    );
                  ...
                  {(["programada", "realizada", "cancelada", "sin_respuesta"] as EstadoTasacion[]).map(estado => {
                    ...
                    return (
                      <div key={estado} style={{
                        ...
                        <div style={{ fontFamily: "Montserrat,sans-serif", fontSize: 22, fontWeight: 800, color: cfg.color }}>{count}</div>
                        <div style={{ fontSize: 10, color: cfg.color, fontFamily: "Montserrat,sans-serif", fontWeight: 700, marginTop: 2 }}>{cfg.label}</div>
                      </div>
                    );
                  })}
                </div>
