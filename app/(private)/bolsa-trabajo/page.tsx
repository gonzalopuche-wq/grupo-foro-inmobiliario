  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto" }}>
      <style>{`
        ...
        .bt-card:hover { border-color: rgba(255,255,255,0.15) !important; background: var(--gfi-border-subtle) !important; }
        .bt-card:hover .bt-title { color: #fff !important; }
        .bt-input:focus { border-color: rgba(153,0,0,0.5) !important; }
        .bt-btn-ghost:hover { color: rgba(255,255,255,0.8) !important; background: var(--gfi-border-subtle) !important; }
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 28, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ fontFamily: "var(--font-display)", fontSize: 22, fontWeight: 800, color: "#fff", margin: 0, display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: 26 }}>💼</span> Bolsa de Trabajo
          </h1>
          <p style={{ fontFamily: "Inter,sans-serif", fontSize: 13, color: "var(--gfi-text-muted)", margin: "6px 0 0" }}>
            Ofertas y búsquedas laborales de la comunidad GFI®
          </p>
        </div>
        <button
          onClick={() => { setForm(EMPTY); setShowForm(true); setDetalle(null); }}
          style={{ background: "#990000", color: "#fff", border: "none", borderRadius: 8, padding: "10px 20px", fontFamily: "var(--font-display)", fontSize: 13, fontWeight: 700, cursor: "pointer", letterSpacing: "0.04em" }}
        >
          + Publicar
        </button>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginBottom: 24 }}>
        {[
          ...
          <div key={s.label} style={{ background: "var(--gfi-bg-card)", border: "1px solid var(--gfi-border-subtle)", borderRadius: 12, padding: "16px", textAlign: "center" }}>
            <div style={{ fontFamily: "var(--font-display)", fontSize: 26, fontWeight: 800, color: "#990000" }}>{s.val}</div>
            <div style={{ fontFamily: "Inter,sans-serif", fontSize: 11, color: "var(--gfi-text-muted)", marginTop: 4 }}>{s.label}</div>
          </div>
        ))}
      </div>
          ...
          {TABS.map(t => (
            <button
              ...
              style={{
                ...
                borderColor: tab === t.key ? "#990000" : "var(--gfi-border)",
                color: tab === t.key ? "#fff" : "var(--gfi-text-secondary)",
              }}
            >
              {t.label}
            </button>
      ...
      {/* Lista */}
      {loading ? (
        <div style={{ textAlign: "center", padding: "48px 0", color: "var(--gfi-text-muted)", fontFamily: "Inter,sans-serif", fontSize: 14 }}>
          Cargando publicaciones...
        </div>
      ) : filtradas.length === 0 ? (
        <div style={{ textAlign: "center", padding: "48px 0" }}>
          <div style={{ fontSize: 48, marginBottom: 16 }}>💼</div>
          <p style={{ fontFamily: "Inter,sans-serif", color: "var(--gfi-text-muted)", fontSize: 14 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {filtradas.map(o => (
            <div
              ...
              style={{ background: "rgba(255,255,255,0.025)", border: "1px solid var(--gfi-border-subtle)", borderRadius: 12, padding: "18px 20px", cursor: "pointer" }}
            >
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 6 }}>
                    <span style={{
                      fontSize: 11, padding: "3px 9px", borderRadius: 20, fontWeight: 600, fontFamily: "var(--font-display)",
                  ...
                  <p style={{ fontFamily: "Inter,sans-serif", fontSize: 13, color: "var(--gfi-text-muted)", margin: "0 0 8px", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                    {o.descripcion}
                  </p>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 12, fontSize: 12, color: "var(--gfi-text-muted)", fontFamily: "Inter,sans-serif" }}>
      ...
      {showForm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: "var(--gfi-bg-secondary)", border: "1px solid var(--gfi-border)", borderRadius: 16, width: "100%", maxWidth: 600, padding: 28, overflowY: "auto", maxHeight: "90vh" }}>
            <h2 style={{ fontFamily: "var(--font-display)", fontSize: 18, fontWeight: 800, color: "#fff", margin: "0 0 20px" }}>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
              <button
                ...
                style={{ padding: "9px 18px", background: "rgba(255,255,255,0.06)", border: "1px solid var(--gfi-border)", borderRadius: 8, color: "var(--gfi-text-secondary)", fontSize: 13, fontFamily: "Inter,sans-serif", cursor: "pointer", transition: "all 0.15s" }}
              >
                Cancelar
              </button>
              <button
                onClick={guardar}
                disabled={guardando}
                style={{ padding: "9px 22px", background: guardando ? "rgba(153,0,0,0.5)" : "#990000", border: "none", borderRadius: 8, color: "#fff", fontSize: 13, fontFamily: "var(--font-display)", fontWeight: 700, cursor: guardando ? "not-allowed" : "pointer" }}
              >
      ...
      {detalle && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: "var(--gfi-bg-secondary)", border: "1px solid var(--gfi-border)", borderRadius: 16, width: "100%", maxWidth: 620, padding: 28, overflowY: "auto", maxHeight: "90vh" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
              <span style={{
                fontSize: 11, padding: "4px 12px", borderRadius: 20, fontWeight: 700, fontFamily: "var(--font-display)",
              ...
              <button onClick={() => setDetalle(null)} style={{ background: "none", border: "none", color: "var(--gfi-text-muted)", fontSize: 20, cursor: "pointer", lineHeight: 1 }}>✕</button>
            </div>

            <h2 style={{ fontFamily: "var(--font-display)", fontSize: 20, fontWeight: 800, color: "#fff", margin: "0 0 8px" }}>
              {detalle.titulo}
            </h2>

            {detalle.perfiles && (
              <p style={{ fontFamily: "Inter,sans-serif", fontSize: 13, color: "var(--gfi-text-muted)", margin: "0 0 16px" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 20 }}>
              {detalle.tipo_contrato && (
                <span style={{ fontSize: 12, padding: "4px 12px", borderRadius: 20, background: "rgba(255,255,255,0.06)", color: "var(--gfi-text-secondary)", fontFamily: "Inter,sans-serif" }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <h3 style={{ fontFamily: "var(--font-display)", fontSize: 11, fontWeight: 700, color: "var(--gfi-text-muted)", letterSpacing: "0.1em", textTransform: "uppercase", margin: "0 0 8px" }}>Descripción</h3>
              <p style={{ fontFamily: "Inter,sans-serif", fontSize: 13, color: "var(--gfi-text-primary)", margin: 0, whiteSpace: "pre-wrap", lineHeight: 1.6 }}>{detalle.descripcion}</p>
            </div>

            {detalle.requisitos && (
              <div style={{ marginBottom: 16 }}>
                <h3 style={{ fontFamily: "var(--font-display)", fontSize: 11, fontWeight: 700, color: "var(--gfi-text-muted)", letterSpacing: "0.1em", textTransform: "uppercase", margin: "0 0 8px" }}>Requisitos</h3>
                <p style={{ fontFamily: "Inter,sans-serif", fontSize: 13, color: "var(--gfi-text-primary)", margin: 0, whiteSpace: "pre-wrap", lineHeight: 1.6 }}>{detalle.requisitos}</p>
              </div>
            )}

            <div style={{ borderTop: "1px solid var(--gfi-border-subtle)", paddingTop: 16 }}>
              <h3 style={{ fontFamily: "var(--font-display)", fontSize: 11, fontWeight: 700, color: "var(--gfi-text-muted)", letterSpacing: "0.1em", textTransform: "uppercase", margin: "0 0 10px" }}>Contacto</h3>
