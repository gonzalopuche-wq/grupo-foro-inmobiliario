...
export default function CotizacionesPage() {
  ...
  const [dolarReferencia, setDolarReferencia] = useState<{
    valor: number | null; proveedor: string | null; compra: number | null; venta: number | null;
  ...
  const publicar = async () => {
    ...
    await supabase.from("divisas_publicaciones").insert({
      perfil_id: userId, tipo: formPub.tipo, moneda: formPub.moneda,
      monto: parseFloat(formPub.monto),
      precio_referencia: formPub.precio_referencia || null,
      zona: formPub.zona || null, notas: formPub.notas || null,
  ...
  const formatHora = (iso: string | null) => iso
    ? new Date(iso).toLocaleTimeString("es-AR", { hour: "2-digit", minute: "2-digit" }) + " · " + new Date(iso).toLocaleDateString("es-AR", { day: "2-digit", month: "2-digit" })
    : null;
              ...
              return (
                <div key={p.id} className={`prov-card${esReferencia ? " referencia" : ""}`}>
                  ...
                  <div className="prov-acciones">
                    ...
                    {esAdmin && (
                      <button className="prov-btn-actualizar" onClick={() => setProveedorActualizando(p)}>
