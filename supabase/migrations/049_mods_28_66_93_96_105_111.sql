...
-- ─────────────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS valoraciones_corredores (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  valorador_id    uuid NOT NULL REFERENCES perfiles(id) ON DELETE CASCADE,
  valorado_id     uuid NOT NULL REFERENCES perfiles(id) ON DELETE CASCADE,
  puntuacion      int NOT NULL CHECK (puntuacion >= 1 AND puntuacion <= 5),
  relacion        text NOT NULL CHECK (relacion IN ('cliente','colega','aliado','comprador','vendedor','otro')),
  comentario      text,
...
CREATE INDEX IF NOT EXISTS idx_val_valorado  ON valoraciones_corredores(valorado_id, visible);
CREATE INDEX IF NOT EXISTS idx_val_valorador ON valoraciones_corredores(valorador_id);

ALTER TABLE valoraciones_corredores ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "val_select_visible" ON valoraciones_corredores;
CREATE POLICY "val_select_visible" ON valoraciones_corredores FOR SELECT
  USING (visible = true OR valorador_id = auth.uid() OR
         EXISTS (SELECT 1 FROM perfiles WHERE id = auth.uid() AND tipo = 'admin'));

DROP POLICY IF EXISTS "val_insert_own" ON valoraciones_corredores;
CREATE POLICY "val_insert_own" ON valoraciones_corredores FOR INSERT
  WITH CHECK (valorador_id = auth.uid());

DROP POLICY IF EXISTS "val_update_own" ON valoraciones_corredores;
CREATE POLICY "val_update_own" ON valoraciones_corredores FOR UPDATE
  USING (valorador_id = auth.uid());

DROP POLICY IF EXISTS "val_delete_own" ON valoraciones_corredores;
CREATE POLICY "val_delete_own" ON valoraciones_corredores FOR DELETE
  USING (valorador_id = auth.uid() OR
         EXISTS (SELECT 1 FROM perfiles WHERE id = auth.uid() AND tipo = 'admin'));
